...
def questionnaire():
    ...
    con = sqlite3.connect("posts.db")  # открываем базу данных
    c = con.cursor()
    for row in c.execute("SELECT post_id, text_post, lenght_post FROM mass_media"):
        if row[2] > 50:
            posts_id_3.append(row[1])
    for row in c.execute("SELECT post_id, text_post, lenght_post FROM tv"):
        if row[2] > 20:
            posts_id_2.append(row[1])
    for row in c.execute("SELECT post_id, text_post, lenght_post FROM gender_relations"):
   ...
   # ques_3 = posts_id_3[4]
    ...
    #lala = random.choice(posts_id_1)
    print(len(posts_id_1), len(posts_id_2), len(posts_id_3))
    urls = {'Главная страница с опросом': url_for('questionnaire'),
            'Все ответы в формате json': url_for('json_Res'),
    ...
    if request.args:
      ...
      #                        quest_1, quest_2, quest_3))
        return render_template('answer.html', name=name, age=age,
                               ...
                               quest_2=quest_2, quest_3=quest_3, urls=urls)
...
def json_Res():
    ...
    return render_template('json.html', json_date=json_date, urls=urls)
...
def stat():
    ...
    for row in c.execute("SELECT * FROM answer"):
        ...
        if row[1] in years:
            ...
            years[row[1]] = 1
    print(lang, city)
    rus_lang = (lang/mid)*100
    mos_city = (city/mid)*100
    con.close()
