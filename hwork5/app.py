...
def new_Datebase():
    ...
    c = con.cursor()
    c.execute(
        "CREATE TABLE IF NOT EXISTS alldata(link text, title text,"
        " ordinary text, lemma text, morph text)")
                ...
                for root3, dirs3, files3 in os.walk(start_path3):
                    for file3 in files3:
                        if ".txt" in file3:
                            ...
                            if file_3 == file_1:
                                with open(final3, 'r', encoding='utf-8') as fe:
                                    morfo = fe.read()
...
def general():
    if request.args:  # для введенного запроса
        ...
        the_end = []
        # поиск по БД
        for row in cursor.execute("SELECT link, title, ordinary,"
                                  " lemma FROM alldata WHERE lemma LIKE ?",
                                  (res, )):
        ...
        #  закрываем БД
        connection.close()
        return render_template('results.html', search1=search1,
                               the_end=the_end)
