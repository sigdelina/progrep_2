...
def searching():
    ...
    if request.args:
        ...
        # выводм информацию
        with open("search.txt", 'w', encoding='utf-8') as f_obj:
            for ele in search_list:
                if word in str(ele) and studying in str(ele):
                    search_str += str(ele).replace('{', '').replace('}', '')
            f_obj.write(search_str)
        #return render_template('items search.html', studying=studying, word=word)
        return render_template('resulting.html', studying=studying, word=word, urls=urls)
